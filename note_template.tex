...
\tcbset{MyThm1/.style={
	fonttitle=\gtfamily\sffamily\bfseries\upshape, % タイトルのフォント
	enhanced,
	colframe=burgundy, % 枠色
	colback=burgundy!2!white, % 中身の背景色
	colbacktitle=burgundy,
	boxrule=0pt, % 枠の線の太さ
	borderline south={2pt}{-2pt}{burgundy}, % 枠の下側の設定
	left*=1zw, % タイトルと本文の左に1zw分のスペース
	right*=1zw, % タイトルと本文の右に1zw分のスペース
	theorem style=standard,
	breakable,sharp corners,
	before skip=10pt, % 枠の前に10ptのスペース
	after skip=10pt, % 枠の後に10ptのスペース
	before upper={\setlength{\parindent}{1zw}}, % 上側の本文に1zwのインデント left*=10zwとの違いは？
	before lower={\setlength{\parindent}{1zw}} % 下側の本文に1zwのインデント(必要無し)
}}
\tcbset{MyThm2/.style={
...
{MyThm1}{lem}
%Definition
\newtcbtheorem[use counter from=thm]{defi}{Definition}{MyThm3}{defi}
%Example
\newtcbtheorem[use counter from=thm]{ex}{Example}{MyThm2}{ex}
%Remark
\newtcbtheorem[use counter from=thm]{rem}{Remark}{MyThm2}{rem}
%行間
\newenvironment{gyoukan}{\begin{leftbar}\color{gray}}{\color{black}\end{leftbar}}
%
%	定理や命題の装飾.
%	\begin{thm}{タイトル}{ラベル名} \end{thm}で定理を書き, \ref{thm:ラベル名}で引用. 
%	しかしTeXShop, VScodeの拡張機能LaTeX Workshopでは, この書き方をするとラベルとして認識してくれない.
%	それを回避するには\begin{thm}{タイトル}{}\label{thm:ラベル名} \end{thm}のように書けばよい.
%	例えば以下を参考.
